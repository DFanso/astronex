...
export class CognitoService {
  ...
  async registerUser(createUserDto: CreateUserDto): Promise<any> {
    ...
    try {
      ...
      createUserDto.cognitoUserId = data.UserSub;
  ...
  async refreshToken(refreshTokenDto: RefreshTokenDto): Promise<any> {
    const secretHash = this.generateSecretHash(refreshTokenDto.cognitoUserId);

    const params = {
      AuthFlow: AuthFlowType.REFRESH_TOKEN_AUTH,
      ClientId: this.clientId,
      AuthParameters: {
        REFRESH_TOKEN: refreshTokenDto.token,
        SECRET_HASH: secretHash,
      },
    };
  ...
  getGoogleSignInUrl(): string {
    ...
    const url = `https://${domain}.auth.${this.configService.get<string>('AWS_REGION')}.amazoncognito.com/oauth2/authorize?client_id=${clientId}&response_type=code&scope=email+openid+profile&redirect_uri=${callbackUrl}`;
    return url;
  }
